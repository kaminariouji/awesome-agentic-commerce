...
- [x402 Audit Agent (crypto-bot-honesty-audit)](https://github.com/kaminariouji/x402-audit-agent) - Pay-per-call x402 API + MCP server that static-audits JS/TS crypto-bot source for the bug patterns that make a bot report income it never earned (testnet-as-USD, silent-zero balances from failed RPCs, cooldown read/write key mismatch). `POST /audit` at 0.05 USDC on Base via x402; free `demo_audit` MCP tool. [Live](https://labored-safari-islamic.ngrok-free.dev) · [x402Scan](https://www.x402scan.com/server/6f86e2b0-87d0-4805-bfd9-7b69be3e1b1d)
